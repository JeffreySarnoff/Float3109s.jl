...
- **Decode** any code point to its exact rational value using `Rational{BigInt}`
...
ValueOf(fmt, 1)            # exact rational value of code point 1
...
| `support.jl`     | `sign_half_offset`, `sign_reduce`, `significand_scale`, `EpsilonOf` |
| `utils.jl`       | `twopow`, `dyadic_twopow` — exact power-of-two helpers |
...
| `values.jl`      | `ValueOf` decoder, `AllFiniteValuesOf`, ordinal accessors |
