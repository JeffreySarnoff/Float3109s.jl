...
| `test_floatvalues.jl` | Boundary values, cross-checks against `ValueOf`, even spacing of subnormals |
| `test_values.jl` | `ValueOf` decoding, monotonicity, negative symmetry, ULP doubling, enumeration lengths |
...
- **Negative symmetry**: `ValueOfOrdinalNeg(fmt, i) == -ValueOfOrdinalPos(fmt, i)`
