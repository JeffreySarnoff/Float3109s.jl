...
their exact rational values using `Rational{BigInt}`.
...
- `cp = 0` returns `0 // 1`.
- NaN, +Inf, and -Inf code points throw `ArgumentError`.
...
```julia
ValueOfOrdinalPos(fmt, i)   # i-th positive finite value (i=1 is smallest)
ValueOfOrdinalNeg(fmt, i)   # i-th negative finite value (i=1 is closest to zero)
```

- `ValueOfOrdinalPos(fmt, 1)` returns the smallest positive value.
- `ValueOfOrdinalPos(fmt, nPosFiniteValuesOf(fmt))` returns the largest
  positive finite value.
- `ValueOfOrdinalNeg` is only valid for signed formats; it throws
...
```
ValueOf(fmt, cp) < ValueOf(fmt, cp + 1)
```
...
```
ValueOfOrdinalNeg(fmt, i) == -ValueOfOrdinalPos(fmt, i)
```
...
v2 = ValueOf(fmt, cp_pos_normal_min(fmt) + 9) - ValueOf(fmt, cp_pos_normal_min(fmt) + 8)
