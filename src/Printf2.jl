...
"""
The `Printf2` module provides formatted output functions similar to the C standard library's `printf2`. It allows formatted printing to an output stream or to a string.
"""
module Printf2
...
"""
    Printf2.Format(format_str)

Create a C printf2-compatible format object that can be used for formatting values.

The input `format_str` can include any valid format specifier character and modifiers.

A `Format` object can be passed to `Printf2.format(f::Format, args...)` to produce a
formatted string, or `Printf2.format(io::IO, f::Format, args...)` to print the
formatted string directly to `io`.

For convenience, the `Printf2.format"..."` string macro form can be used for building
a `Printf2.Format` object at macro-expansion-time.

!!! compat "Julia 1.6"
    `Printf2.Format` requires Julia 1.6 or later.
"""
struct Format{S,T}
...
function Format(f::AbstractString)
    ...
    while pos <= len
        ...
        elseif b in b"Ljqtz" # q was a synonym for ll above, see `man 3 printf2`. Not to be used.
            pos > len && throw(InvalidFormatStringError("Length modifier is missing type specifier", f, last_percent_pos, pos - 1))
            b = bytes[pos]
            pos += 1
        end
...
Convert an argument to a Base float type for printf2 formatting.
...
that wish to hook into printf2 formatting can extend this method like:

```julia
Printf2.tofloat(x::MyCustomType) = convert_my_custom_type_to_float(x)
```

For arbitrary precision numerics, you might extend the method like:

```julia
Printf2.tofloat(x::MyArbitraryPrecisionType) = BigFloat(x)
```
        ...
        if isfinite(x)
            GC.@preserve buf begin
                ...
                if required_length > siz
                    ...
                    required_length = _snprintf2(pointer(buf, pos), required_length + 1, str, x)
                end
...
"""
    Printf2.format(f::Printf2.Format, args...) => String
    Printf2.format(io::IO, f::Printf2.Format, args...)

Apply a printf2 format object `f` to provided `args` and return the formatted string
(1st method), or print directly to an `io` object (2nd method). See [`@printf2`](@ref)
for more details on C `printf2` support.
"""
function format end
...
"""
    @printf2([io::IO], "%Fmt", args...)

Print `args` using C `printf2` style format specification string.
Optionally, an `IO` may be passed as the first argument to redirect output.

# Examples
```jldoctest
julia> @printf2 "Hello %s" "world"
Hello world

julia> @printf2 "Scientific notation %e" 1.234
Scientific notation 1.234000e+00

julia> @printf2 "Scientific notation three digits %.3e" 1.23456
Scientific notation three digits 1.235e+00

julia> @printf2 "Decimal two digits %.2f" 1.23456
Decimal two digits 1.23

julia> @printf2 "Padded to length 5 %5i" 123
Padded to length 5   123

julia> @printf2 "Padded with zeros to length 6 %06i" 123
Padded with zeros to length 6 000123

julia> @printf2 "Use shorter of decimal or scientific %g %g" 1.23 12300000.0
Use shorter of decimal or scientific 1.23 1.23e+07

julia> @printf2 "Use dynamic width and precision  %*.*f" 10 2 0.12345
Use dynamic width and precision        0.12
```
For a systematic specification of the format, see [here](https://en.cppreference.com/w/c/io/fprintf2).
See also [`@sprintf2`](@ref) to get the result as a `String` instead of it being printed.
...
# Examples
```jldoctest
julia> @printf2("%f %F %f %F", Inf, Inf, NaN, NaN)
Inf Inf NaN NaN

julia> @printf2 "%.0f %.1f %f" 0.5 0.025 -0.0078125
0 0.0 -0.007812
```
...
"""
macro printf2(io_or_fmt, args...)
    if io_or_fmt isa String
        fmt = Format(io_or_fmt)
        return esc(:($Printf2.format(stdout, $fmt, $(args...))))
    else
        io = io_or_fmt
        isempty(args) && throw(ArgumentError("No format string provided to `@printf2` - use like `@printf2 [io] <format string> [<args...>]."))
        fmt_str = first(args)
        fmt_str isa String || throw(ArgumentError("First argument to `@printf2` after `io` must be a format string"))
        fmt = Format(fmt_str)
        return esc(:($Printf2.format($io, $fmt, $(Base.tail(args)...))))
    end
end
...
Return [`@printf2`](@ref) formatted output as string.

# Examples
```jldoctest
julia> @sprintf2 "this is a %s %15.1f" "test" 34.567
"this is a test            34.6"
```
"""
macro sprintf2(fmt, args...)
    fmt isa String || throw(ArgumentError("First argument to `@sprintf2` must be a format string."))
    f = Format(fmt)
...
function _normalize_a_string_to_leading_one_legacy(s::AbstractString)
    ...
    body = SubString(s, i)
    startswith(body, "0x") || return s

    p = findfirst('p', body)
    p === nothing && return s

    mantissa = body[3:prevind(body, p)]
    exponent = body[nextind(body, p):end]
    dot = findfirst('.', mantissa)
    intpart = dot === nothing ? mantissa : mantissa[1:prevind(mantissa, dot)]
    fracpart = dot === nothing ? "" : mantissa[nextind(mantissa, dot):end]
    hexdigits = string(intpart, fracpart)
    isempty(hexdigits) && return s

    digits = parse(BigInt, hexdigits; base=16)
    digits == 0 && return s

    exp = parse(Int, exponent)
    e2 = exp - 4 * length(fracpart)
    shift = ndigits(digits, base=2) - 1
    enew = e2 + shift

    rem = digits - (big(1) << shift)
    fracnew = "0"
    if rem != 0
        ndigits_hex = cld(shift, 4)
        scaled = rem << (4 * ndigits_hex - shift)
        fracnew = lpad(string(scaled, base=16), ndigits_hex, '0')
        fracnew = rstrip(fracnew, '0')
        isempty(fracnew) && (fracnew = "0")
    end

    mantnew = fracnew == "0" ? "1" : string("1.", fracnew)
    expnew = enew >= 0 ? string("+", enew) : string(enew)
...
function _normalize_a_string_to_leading_zero_legacy(s::AbstractString)
    ...
    exp = parse(Int, exponent)
    fracnew = string(digits, base=16)
    expnew = exp - 4 * length(fracpart) + 4 * length(fracnew)
...
"""
    sprintf2a(args...; subnormal=false)

Return `%a` formatted output as string with either normal (`0x1._`) or
