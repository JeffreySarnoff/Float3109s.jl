...
@testset "val_pos_normal_max: extended < finite (same K,P)" begin
    ...
    for K in 3:8, P in 2:(K - 1)
        for S in (is_unsigned, is_signed)
            Σ = S === is_signed ? 1 : 0
            K - P + 1 - Σ >= 1 || continue
            ff = Format{S, is_finite}(K, P)
            fe = Format{S, is_extended}(K, P)
